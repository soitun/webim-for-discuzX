...
g.init("unicast",p.id,p.history)});f.online(k);f.handle(c.buddies);L(c.rooms,function(r,p){g.init("multicast",p.id,p.history)});f=this.setting.get("blocked_rooms");var q=c.rooms;ba(f)&&q&&L(f,function(r,p){q[p]&&(q[p].blocked=true)});i.handle(q);i.options.ticket=c.connection.ticket;this.trigger("go",[c]);this.connection.connect(c.connection);if((c=c.new_messages)&&c.length){L(c,function(r,p){p["new"]=true});this.trigger("message",[c])}},_stop:function(c){w.onbeforeunload=this._unloadFun;this.data.user.presence=
...
this.dataHash[c];if(g&&!g.initMember){g.initMember=true;this.loadMember(c)}},loadMember:function(c){var g=this,f=g.options;C({type:"get",cache:false,url:f.urls.member,dataType:"json",data:{ticket:f.ticket,id:c},success:function(i){g.addMember(c,i)}})},join:function(c,g){var f=this,i=f.options;C({cache:false,type:"post",url:i.urls.join,dataType:"json",data:{ticket:i.ticket,id:c,nick:g.nick},success:function(k){f.initMember(c);f.handle([k])}})},leave:function(c,g){var f=this.options,i=this.dataHash[c];
if(i){i.initMember=false;C({cache:false,type:"post",url:f.urls.leave,data:{ticket:f.ticket,id:c,nick:g.nick}});this.trigger("leave",[i])}},clear:function(){}})})();t("history",{urls:{load:"webim/history",clear:"webim/clear_history"}},{_init:function(){this.data=this.data||{};this.data.unicast=this.data.unicast||{};this.data.multicast=this.data.multicast||{}},unicast:function(c){return this.data.unicast[c]},multicast:function(c){return this.data.multicast[c]},handle:function(c){var g=this.data,f={unicast:{},
multicast:{}};c=E(c);var i=c.length,k,o,q=this.options.userInfo.id;if(i){for(var r=0;r<i;r++){k=c[r];p=k.type;if((o=p=="unicast"?k.to==q?k.from:k.to:k.to)&&p){f[p][o]=f[p][o]||[];f[p][o].push(k)}}for(var p in f)for(o in f[p]){k=f[p][o];if(g[p][o]){g[p][o]=g[p][o].concat(k);this._triggerMsg(p,o,k)}else this.load(p,o)}}},_triggerMsg:function(c,g,f){this.trigger(c,[g,f])},clear:function(c,g){var f=this.options;this.data[c][g]=[];this.trigger("clear",[c,g]);C({url:f.urls.clear,type:"post",cache:false,
data:{type:c,id:g}})},init:function(c,g,f){if(ba(f)){this.data[c][g]=f;this._triggerMsg(c,g,f)}},load:function(c,g){var f=this,i=f.options;f.data[c][g]=[];C({url:i.urls.load,cache:false,type:"get",dataType:"json",data:{type:c,id:g},success:function(k){f.init(c,g,k)}})}})})(window,document);
...
na(b.prev);s(b.expand,"click",function(){if(!a.isMinimize())return false;a.expand();a.trigger("expand");return false});s(b.collapse,"click",function(){if(a.isMinimize())return false;a.collapse();a.trigger("collapse");return false});L(b.collapse,"ui-state-hover","ui-state-default");L(b.expand,"ui-state-hover","ui-state-default")},isMinimize:function(){return Y(this.$.layout,"webim-layout-minimize")},collapse:function(){this.isMinimize()||D(this.$.layout,"webim-layout-minimize")},expand:function(){this.isMinimize()&&
...
this;b=f(b,{closeable:false,subHeader:a.header});var j=a.element;b=new x.window(null,b);b.html(j);h.$[e?e:"widgets"].insertBefore(b.element,d&&h.widgets[d]?h.widgets[d].window.element:null);a.window=b;b.bind("displayStateChange",function(l){h._widgetStateChange(this,l)});h.widgets[a.name]=a},focusChat:function(a,b){b=ia(a,b);var d=this.tabs[b],e=this.panels[b];d&&d.isMinimize()&&d.restore();e&&e.focus()},chat:function(a,b){return this.panels[ia(a,b)]},updateChat:function(a,b){b=g(b);for(var d,e=b.length,
h,j=0;j<e;j++){d=b[j];(h=this.panels[ia(a,d.id)])&&h.update(d)}},updateAllChat:function(){i(this.panels,function(a,b){b.update()})},_onChatClose:function(a){this.tabIds=k(this.tabIds,function(b){return b!=a});delete this.tabs[a];delete this.panels[a];this._changeActive(a,true);this._fitUI()},_onChatChange:function(a,b){if(b=="minimize"){this._changeActive(a,true);this._displayUpdate()}else{this._changeActive(a);this._fitUI()}},_changeActive:function(a,b){var d=this.activeTabId;if(b)d==a&&(this.activeTabId=
null);else{d&&d!=a&&this.tabs[d].minimize();this.activeTabId=a;this._updatePrevCount(a)}},addChat:function(a,b,d,e){var h=this,j=h.panels,l=b.id;l=ia(a,l);if(!j[l]){a=h.tabs[l]=(new x.window(null,f({isMinimize:h.activeTabId||!h.options.chatAutoPop,tabWidth:h.tabWidth-2},e))).bind("close",function(){h._onChatClose(l)}).bind("displayStateChange",function(m){h._onChatChange(l,m)});h.tabIds.push(l);h.$.tabs.insertBefore(a.element,h.$.tabs.firstChild);j[l]=new x.chat(null,f({window:a,user:h.options.user,
info:b},d));!a.isMinimize()&&h._changeActive(l);h._fitUI()}},removeChat:function(a,b){var d=this.tabs[ia(a,b)];d&&d.close()},removeAllChat:function(){i(this.tabs,function(a,b){b.close()})},addShortcut:function(a){var b=this;if(P(a))i(a,function(h,j){b.addShortcut(j)});else if($(a)){var d=b.$.shortcut,e=b.options.tpl_shortcut;if(!(d.childNodes.length>b.options.shortcutLength+1)){e=H(G(e,{title:t(a.title),icon:a.icon,link:a.link,target:a.isExtlink?"_blank":""}));L(e.firstChild,"ui-state-hover");d.appendChild(e)}}},
addWindow:function(){new x.window(null,{})},online:function(){},offline:function(){}});K("history",{user:{},info:{},template:'<div class="webim-history">                        <div id=":content" class="webim-history-content">                 </div></div>'},{_init:function(){M.call(this,"init",[null,this.ui()])},clear:function(){this.$.content.innerHTML="";this.trigger("clear")},add:function(a){a=g(a);var b=a.length,d=[];if(b){for(var e=0;e<b;e++)d.push(this._renderMsg(a[e]));this.$.content.innerHTML+=
d.join("");this.trigger("update")}},_renderMsg:function(a){a=f({},a);M.call(this,"render",[null,this.ui({msg:a})]);var b=a.from,d=a.to,e=a.timestamp,h=a.body,j=true,l=this._lastLogItem,m=[],n;n=a.nick;if(l&&l.to==d&&l.from==b&&e-l.timestamp<6E4)j=false;if(j){this._lastLogItem=a;a=new R(e);m.push('<h4 class="ui-state-default"><span class="webim-gray">');m.push(a.getDay(true));m.push(" ");m.push(a.getTime());m.push("</span>");m.push(n);m.push("</h4>")}m.push("<p>");m.push(h);m.push("</p>");return m.join("")},
_renderDateBreak:function(a){var b=this._lastLogItem,d=new Date,e=new Date,h=[];d.setTime(a);b&&e.setTime(b.timestamp);if(!b||d.getDate()!=e.getDate()||d.getMonth()!=e.getMonth()){h.push("<h5>");h.push((new R(a)).getDay(true));h.push("</h5>")}return h.join("")},ui:function(a){return f({element:this.element,$:this.$},a)},plugins:{}});var ra=function(){function a(e,h){return'<a href="'+(h=="www."?"http://"+e:e)+'"'+d+">"+e+"</a>"}function b(e,h){d+=" "+e+'="'+h+'"'}var d;return function(e,h){d="";h&&
$(h)&&i(h,b);return e.replace(/(https?:\/\/|www\.)([^\s<]+)/ig,a)}}();x.history.defaults.parseMsg=true;M.add("history","parseMsg",{render:function(a,b){var d=b.msg.body;d=V(d);d=ra(d,{target:"_blank"});b.msg.body=d}});x.history.defaults.emot=true;M.add("history","emot",{render:function(a,b){b.msg.body=x.emot.parse(b.msg.body)}});K("emot",{template:'<div class="webim-emot ui-widget-content"><%=emots%></div>'},{_init:function(){var a=this,b=a.element;i(b.firstChild.childNodes,function(d,e){s(e,"click",
function(){E(b,"webim-emot-show");a.trigger("select",this.firstChild.getAttribute("alt"))})})},template:function(){var a=this.emots=webim.ui.emot.emots,b=[];b.push('<ul class="ui-helper-clearfix">');i(a,function(d,e){var h=e.src,j=e.t?e.t:e.q[0];b.push('<li><img src="');b.push(h);b.push('" title="');b.push(j);b.push('" alt="');b.push(e.q[0]);b.push('" /></li>')});b.push("</ul>");return G(this.options.template,{emots:b.join("")})},toggle:function(){ha(this.element,"webim-emot-show")}});f(x.emot,{emots:[{t:"smile",
...
b&&i(b,function(e,h){var j=d[h],l=j.src,m=j.t?j.t:j.q[0],n=[];n.push('<img src="');n.push(l);n.push('" title="');n.push(m);n.push('" alt="');n.push(j.q[0]);n.push('" />');e=V(e);e=e.replace(RegExp("(\\"+".$^*\\[]()|+?{}:<>".split("").join("|\\")+")","g"),"\\$1");a=a.replace(RegExp(e,"g"),n.join(""))});return a}});K("chat",{tpl_header:'<div><div id=":user" class="webim-user"> \t\t\t<a id=":userPic" class="webim-user-pic ui-corner-all ui-state-active" href="#id"><img width="50" height="50" src="about:blank" defaultsrc="" onerror="var d=this.getAttribute(\'defaultsrc\');if(d && this.src!=d)this.src=d;" class="ui-corner-all"></a> \t\t\t<span id=":userStatus" title="" class="webim-user-status">&nbsp;</span> \t\t     </div></div>',
